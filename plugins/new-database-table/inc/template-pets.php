...
<div class="container container--narrow page-section">
  ...
  <p>This page took <strong><?php echo timer_stop(); ?></strong> seconds to prepare. Found
    <strong><?php echo number_format($getPets->count); ?></strong> results (showing the first <?php echo count($getPets->pets) ?>).</p>
        ...
        <?php if (current_user_can('administrator')) { ?>
          <td style="text-align: center;">
            <form action="<?php echo esc_url(admin_url('admin-post.php')) ?>" method="POST">
              <input type="hidden" name="action" id="" value="deletepet">
              <input type="hidden" name="idtodelete" id="" value="<?php echo $pet->id ?>">
  ...
  if (current_user_can('administrator')) { ?>
    <form action="<?php echo esc_url(admin_url('admin-post.php')) ?>" class="create-pet-form" method="POST">
      <p>Enter just the name for a new pet. Its species, weight and other details with a rendomaly generated.</p>
      <input type="hidden" name="action" id="" value="createpet">
      <input type="text" name="incomingpetname" placeholder="name...." id="">
